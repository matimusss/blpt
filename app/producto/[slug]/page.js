...
function replacePlaceholders(htmlString, data) {
  return htmlString.replace(/{{(.*?)}}/g, (_, key) => {
    if (key.trim() === 'images_component') {
      return '<div id="images_component"></div>'; // Marcador temporal
    }
    return data[key.trim()] || ''; // Reemplaza los demás marcadores de posición con los valores del objeto de datos
  });
}
